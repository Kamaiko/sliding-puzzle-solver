...
**Technique de recherche.** Nous avons utilisé l'algorithme A* parce qu'il garantit de trouver la solution optimale. A* fonctionne en combinant le coût réel g(n) depuis le départ avec une estimation heuristique h(n) du coût restant, pour calculer f(n) = g(n) + h(n). L'heuristique de distance Manhattan additionne pour chaque tuile le nombre de déplacements horizontaux et verticaux nécessaires pour atteindre sa position finale. Cette heuristique est admissible (ne surestime jamais le coût réel) et consistante (respecte l'inégalité triangulaire). L'algorithme utilise une liste ouverte pour les nœuds à explorer (triée par f(n) croissant) et un ensemble fermé pour éviter de revisiter les mêmes états.
